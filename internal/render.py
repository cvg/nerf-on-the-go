...
def volumetric_rendering_nerfw(
                        rgbs_static,
                        rgbs_transient,
                        trans,
                        alpha_static,
  ...
  eps = jnp.finfo(jnp.float32).eps
  rendering = {}
  weights_static = trans * alpha_static
  weights_transient = trans * alpha_transient
  weights = weights_static + weights_transient

  acc = (weights_static + weights_transient).sum(axis=-1)
  bg_w = jnp.maximum(0, 1 - acc[..., None])  # The weight of the background.
  rgb = (
    weights_static[..., None] * rgbs_static  + 
    weights_transient[..., None] * rgbs_transient
  ).sum(axis=-2) + bg_w * bg_rgbs
  rendering['rgb'] = rgb
