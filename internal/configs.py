...
class Config:
  ...
  dataset_loader: str = 'on-the-go'  # The type of dataset loader to use.
  ...
  forward_facing: bool = False  # Set to True for forward-facing captures.
  render_path: bool = False  # If True, render a path. 
  ...
  data_loss_type: str = 'dino_ssim'  # What kind of loss to use ('mse' or 'charb').
  charb_padding: float = 0.001  # The padding used for Charbonnier loss.
  data_loss_mult: float = 0.5  # Mult for the finest data term in the loss.
  ...
  # weight_decay_mults:FrozenDict[str, Any] = FrozenDict({
  #       'NerfMLP_0': 0.00001,
  ...
  is_render: bool = False # whether is rendering, for feature assignment bug
  ...
  # dino configs
  dino_var_mult: float = 0.1 # multiplier for the variance of the dino features as regularization
  compute_feature_metrics: bool = True  # If True, compute feature.
  feat_rate: int = 4  # Feature sampling rate w.r.t original image size.
  dilate: int = 8  # The dilate rate for the patch size
  eval_train: bool = True # evaluate test set or train set, for debug
  train_clean: bool = False # train on clean set or clutter set
  reg_mult: float = 0.5 # reg weight for uncertainty reg
  uncer_lr_rate: float = 1 # lr rate w.r.t nerf 
  uncer_clip_min: float = 0.1 # minimum value to clip the uncertainty
  ssim_clip_max: float = 5 # maximum value to clip the ssim
  ssim_mult: float = 0.5 #multiplicative factor for ssim loss 
  H: int = 3024 #height of the image
  W: int = 4032 #weight of the image
  ssim_anneal: float= 0.8 # anneal rate for ssim
  stop_ssim_gradient: bool = True # whether to stop the gradient flow from ssim to reconstruction
  ssim_window_size: int = 5 # window size of ssim
  mask_type: str  = 'masks' # use the mask of which folder 
  feat_dim: int = 384 # feature dimension, 384 for dino_s/14
  feat_ds: int = 14 # feature downsample rate for dino, 14 for dino_s/14, combine together with feat_rate
