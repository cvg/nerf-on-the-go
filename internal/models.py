...
class Model(nn.Module):
  ...
  num_dino_features: int = 0 # dino embedding length
  num_dino_embeddings: int = 1000
  ...
  def __call__(
    ...
    if self.num_dino_features > 0:
      if not zero_dino:
        # Construct/grab dino vectors for the cameras of each input ray.
        dino_vecs = nn.Embed(self.num_dino_embeddings, self.num_dino_features)
        cam_idx = rays.cam_idx[..., 0]
        dino_vec = dino_vecs(cam_idx)
      else:
        dino_vec = jnp.zeros(rays.origins.shape[:-1] + (self.num_dino_features,))
    else:
      dino_vec = None
    ...
    for i_level in range(self.num_levels):
      ...
      uncer = uncer_mlp(rays.features, is_training=is_training)
      
      #  uncertainty regularization, eq (2) & (3)
      if uncer.shape[0] > 1 and len(uncer.shape) > 2 and self.config.dino_var_mult > 0:
        dim0, dim1, dim2, _ = uncer.shape
        uncer = uncer.reshape(-1, 1)

        # compute affinity matrix
        feat_flatten = rays.features.reshape(-1, self.config.feat_dim)
        feat_norm =  (feat_flatten / jnp.linalg.norm(feat_flatten, axis=-1, keepdims=True))
        affinity = jnp.matmul(feat_norm, jnp.transpose(feat_norm, (-1, -2)))

        # only keep the top 128 similar features with similarity > 0.75
        topk_value, topk_ind = lax.top_k(affinity, k=128)
        value_mask = (topk_value > 0.75)


        # calculate eq(2) & eq(3) in the paper
        uncer_nn = uncer[topk_ind].squeeze() * jnp.array(value_mask, dtype=jnp.float32)
        sums = jnp.sum(uncer_nn * jnp.array(value_mask, dtype=jnp.float32), axis=-1)
        counts = jnp.sum(value_mask, axis=-1) + 1e-6
        uncer_means = sums / counts
        squared_diffs = (uncer_nn - uncer_means[..., jnp.newaxis]) ** 2
        uncer_variances = jnp.sum(squared_diffs, axis=-1) / counts
        
        uncer_means = uncer_means.reshape(dim0, dim1, dim2, 1)
        uncer_variances= uncer_variances.reshape(dim0, dim1, dim2, 1)

      else:
        uncer_means = uncer
        uncer_variances = jnp.zeros_like(uncer_means)
      ...
      rendering['uncer'] = uncer_means
      rendering['uncer_var'] = uncer_variances
      if compute_extras:
...
class MLP(nn.Module):
  ...
  transient_width: int = 128
  net_width_transient: int = 2
...
class UncerMLP(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, dino_emb = None, is_training=True):
        dense_layer = functools.partial(nn.Dense, 
                                        kernel_init=getattr(jax.nn.initializers, self.weight_init)())
        if dino_emb is not None:
          x = jnp.concatenate([x, dino_emb], axis=-1)
        for i in range(self.net_depth):
          x = dense_layer(self.hidden_dim)(x)
          x = self.net_activation(x)
          x = nn.Dropout(rate=0.25,deterministic=not is_training)(x)
        x = dense_layer(self.output_dim)(x)
        x = nn.softplus(x)
...
def render_image(render_fn: Callable[[jnp.array, utils.Rays],
  ...
  for k, z in rendering.items():
    if not k.startswith('ray_') and z is not None:
