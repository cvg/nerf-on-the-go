...
class MetricHarnessLPIPS:
  """A helper class for evaluating several error metrics with vgg16 lpips."""

  def __init__(self):
    self.ssim_fn = jax.jit(dm_pix.ssim)
    self.lpips_fn = lpips_jax.LPIPSEvaluator(replicate=False, net='vgg16') # ['alexnet', 'vgg16']

  def __call__(self, rgb_pred, rgb_gt, name_fn=lambda s: s):
    """Evaluate the error between a predicted rgb image and the true image."""
    psnr = float(mse_to_psnr(((rgb_pred - rgb_gt)**2).mean()))
    ssim = float(self.ssim_fn(rgb_pred, rgb_gt))
    # To fix lpips calculation, now it's broken
    # lpips = float(self.lpips_fn(rgb_pred[None,...]*2-1, rgb_gt[None,...]*2-1))
